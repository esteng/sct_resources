...
word type information gotten from wiktionary (https://en.wiktionary.org/wiki/Category:Arabic_lemmas)
...
def get_function_words():
	functionwords = []
	conjhtml = str(getHTML('https://en.wiktionary.org/wiki/Category:Arabic_conjunctions'))
	prephtml = str(getHTML('https://en.wiktionary.org/wiki/Category:Arabic_prepositions'))
	pronhtml = str(getHTML('https://en.wiktionary.org/wiki/Category:Arabic_pronouns'))
	...
	for word in functionwords:
		if 'Arabic' in word or 'lemmas' in word or 'by' in word or word == 'Prepositions by language':
...
getFrequency('ar_full.txt')
f2 = open('ArabicEnrichmentData.csv', 'w')
f2CW = csv.writer(f2)
