...
word type information gotten from wiktionary (https://en.wiktionary.org/wiki/Category:Vietnamese_lemmas)
...
def get_function_words():
	functionwords = []
	conjhtml = str(getHTML('https://en.wiktionary.org/wiki/Category:Vietnamese_conjunctions'))
	prephtml = str(getHTML('https://en.wiktionary.org/wiki/Category:Vietnamese_prepositions'))
	pronhtml = str(getHTML('https://en.wiktionary.org/wiki/Category:Vietnamese_pronouns'))
	...
	for word in functionwords:
		if 'Vietnamese' in word or 'lemmas' in word or 'by' in word or word == 'Prepositions by language':
...
getFrequency('vi_full.txt')
f2 = open('VietnameseEnrichmentData.csv', 'w')
f2CW = csv.writer(f2)
